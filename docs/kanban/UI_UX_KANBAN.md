# UI/UX Kanban

Markers:
- `[ ]` Backlog
- `[-]` In Progress
- `[!]` Blocked
- `[x]` Done

Reference: `docs/tasks/UI_UX_TASKS.md`

---

## Backlog
- [ ] `UX-4002` Profile save feedback and lock behavior refinement
- [ ] `UX-5001` Responsive regression test pass (re-open after stabilization)
- [ ] `UX-5002` Accessibility smoke checks (re-open after stabilization)

## In Progress
- [-] `UX-0002` Define component style contract
- [-] `UX-1001` Responsive header/nav redesign
- [-] `UX-1002` Global typography and spacing normalization
- [-] `UX-1003` Accessibility baseline pass
- [-] `UX-2001` Test page fatigue reduction
- [-] `UX-2002` Results page hierarchy and interaction polish
- [-] `UX-2003` Chatbot conversation UX improvements
- [-] `UX-2004` VR recommendation emphasis cleanup
- [-] `UX-3001` Teacher import panel redesign
- [-] `UX-3003` Dashboard readability pass
- [-] `UX-4001` Auth forms inline validation messaging

## Blocked
- No blocked items

## Done
- [x] `UX-0001` Create page-by-page UX spec
- [x] `UX-3002` Import feedback and error rendering improvements
- [x] `UX-9001` Frontend runtime consolidation
- [x] `UX-5003` Update docs with UX behavior changes
- [x] UI audit and problem identification completed
- [x] Planning artifacts created (`docs/plans/UI_UX_PLAN.md`, `docs/tasks/UI_UX_TASKS.md`, `docs/kanban/UI_UX_KANBAN.md`)

---

## Suggested WIP Limits
- Frontend implementation: max 3 tickets
- QA validation: max 2 tickets
